...
# Source: src/nvitk/measure/morpho/anatomy_axes.py
...
dependency of :mod:`nvitk_vendor.measure.morpho.io_utils`) so that
:mod:`nvitk_vendor.measure.morpho` stays importable without the rest of nvitk_vendor — the
Slicer ``MouseTOFMorphometrics`` module relies on that.

.. note::
   :class:`AnatomicalAxes` intentionally mirrors ``RasAxes`` /
   ``resolve_ras_axes`` in
   :mod:`nvitk_vendor.pipes.qvtpy.util.centerline.venous_heuristics`. It is not shared
...
class MorphoContext:
    ...
    Frozen and picklable: :func:`nvitk_vendor.measure.morpho.run_case.run_case` spawns
