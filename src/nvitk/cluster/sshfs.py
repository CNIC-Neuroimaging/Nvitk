...
def _iter_proc_mounts() -> Iterator[tuple[str, str]]:
    """Yield ``(mountpoint, fstype)`` for every current mount."""
    try:
...
def is_mounted(mountpoint: str | Path) -> bool:
    ...
    return any(
        ...
        for mount_point, fstype in _iter_proc_mounts()
    )
...
def active_mounts() -> list[SshfsMount]:
    """Mounts this process is currently holding open (diagnostics and leak checking)."""
    with _REGISTRY_LOCK:
...
def cluster_mount(
    *,
    ...
    Reference counted per ``(user, host, remote_root)``. A per-subject loop that opens the
    same root a thousand times mounts once and unmounts once; without this, each iteration
    would pay a full SSH handshake and FUSE setup. Only mounts this process created are
    unmounted, and only once the last holder has left.
    """
    root = _normalise_remote(remote_root)
    key = (user, host, root)
    with _REGISTRY_LOCK:
