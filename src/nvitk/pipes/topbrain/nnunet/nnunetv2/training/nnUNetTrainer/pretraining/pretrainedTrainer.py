...
class PretrainedTrainer(nnUNetTrainer):
    ...
    def load_pretrained_weights(
        ...
        lpe_in_stem = False
        # nvitk fix: lpe_in_encoder was only bound inside the `key_to_lpe is not None` branch
        # below, yet is read unconditionally further down. Convolutional encoders (ResEnc-L) have
        # no learnable positional embedding, so key_to_lpe is None for them and the reads raised
        # UnboundLocalError. Primus checkpoints carry one, which is why only the CNN path failed.
