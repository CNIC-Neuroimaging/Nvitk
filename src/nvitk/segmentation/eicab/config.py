...
# Fast host scratch for eICAB ``/tmp`` bind (per-subject subdirs via
# :func:`resolve_eicab_tmp_dir`). Falls back to ``<output>/.eicab_tmp`` when missing.
# Override via ``pipelines.eicab.default_tmp_dir`` in ``.nvitk/sge.json``.
DEFAULT_TMP_DIR = Path("/data_tmp")
...
SGE_H_VMEM = "25G"
SGE_QUEUE = None
SGE_LOG_DIR = Path("/data3/BIOIT_IMAGE/nvitk-sge/SGE_SCRIPTS/logs/eICAB")
SGE_ERR_DIR = Path("/data3/BIOIT_IMAGE/nvitk-sge/SGE_SCRIPTS/errs/eICAB")

DEFAULT_SGE_SCRIPTS_DIR: Path = Path("/data3/BIOIT_IMAGE/nvitk-sge/SGE_SCRIPTS/")

# Host tree bind-mounted to ``/programs/Neuro/vasculature2`` (legacy eICAB layout).
DEFAULT_VASCULATURE_HOST_DIR = Path("/programs/Neuro/vasculature2")
